...
Shader "Ciconia Studio/Effects/MirrorReflection/Forward rendering/Fast Window" {
    Properties {
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform fixed _UseDirtmapasTransparencymap;
...
////// Specular:
                ...
                float3 CubemapSpec = (_EnableCubemap*((((0.95*pow(1.0-max(0,dot(normalDirection, viewDirection)),1.0))+0.05)*_FresnelStrength)+(_Cubemap_var.rgb*(_Cubemap_var.a*_CubemapIntensity)*node_2724)));
...
/////// Diffuse:
                ...
                float2 node_862 = float2(sceneUVs.r,(1.0 - sceneUVs.g));
                float4 _ReflectionTex_var = tex2D(_ReflectionTex,TRANSFORM_TEX(node_862, _ReflectionTex));
                float node_1899 = 1.0;
                float CubemapActive = _EnableCubemap;
                float3 node_1109 = (lerp(_Color.rgb,_DirtTint.rgb,node_3386)*_ReflectionTex_var.rgb*lerp(float3(node_1899,node_1899,node_1899),(_ReflectionTex_var.a+saturate(((1.0 - _ReflectionTex_var.a)*CubemapSpec))),CubemapActive));
                float3 Albedo = lerp( (node_1109*node_2724*node_2724), saturate((node_1109+node_3386)), _ScreenBlendingmode );
                float3 diffuseColor = Albedo;
                diffuseColor *= 1-specularMonochrome;
                float3 diffuse = (directDiffuse + indirectDiffuse) * diffuseColor;
/// Final Color:
                float3 finalColor = diffuse + specular;
                float node_79 = lerp(1,0,_Transparency);
                fixed4 finalRGBA = fixed4(finalColor,lerp( node_79, (node_2724*node_79), _UseDirtmapasTransparencymap ));
...
/// Final Color:
                float3 finalColor = diffuse + specular;
                float node_79 = lerp(1,0,_Transparency);
                fixed4 finalRGBA = fixed4(finalColor * lerp( node_79, (node_2724*node_79), _UseDirtmapasTransparencymap ),0);
                UNITY_APPLY_FOG(i.fogCoord, finalRGBA);
                return finalRGBA;
            }
            ENDCG
